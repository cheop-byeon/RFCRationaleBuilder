...
class GithubAPI:
          ...
          repository(owner: $owner, name: $repo) {
            ...
            pullRequests(
              ...
              nodes {
                ...
                latestReviews(first: $pullRequestPerPage) {
                    nodes{
                        body
                    }
                }
    ...
    def _parse_commit(
        ...
        for commit in commits:
            ...
            pr_nodes = [node for node in commit["associatedPullRequests"]["nodes"] if node]
            associatedPullRequests_ids = [node['id'] for node in pr_nodes]
            associatedPullRequests_numbers = [node['number'] for node in pr_nodes]
            parents_ids = [node["oid"] for node in commit.get("parents", {}).get("nodes", [])]
    ...
    def _parse_issue_or_pull_request(
            ...
            try:
                comments.append(
                    GithubComment(
                        ...
                        created_via_email=c.get("createdViaEmail", False),
                        body=c["body"],
                        user_login=c.get("author", {}).get("login", "(unknown)"),
                        user_url=c.get("author", {}).get("url", "(unknown)"),
                        url=c["url"],
                    )
                )
            except Exception:
        ...
        return GithubIssue(
            ...
            latestReviews = [node["body"] for node in i["latestReviews"]["nodes"] if node not in [None, {}]] if is_pull_request else []
        )
...
def export_issues_to_csv(
    ...
    headers = ['number', 'type','state', 'pr_commits', 'merged_commits', 'parents', 'issues', 'comments']
    fileName = output_file_name 
    # it should be merged, otherwise it does not exist locally
    prs =  [issue for issue in repo.issues if issue.state == 'merged' and issue.pull_request] # PRs
    issues = [issue for issue in repo.issues if issue.state == 'closed' and not issue.pull_request] # ISSUES
    closingIssues = []
    ...
    with open(fileName, 'w', newline='') as file:
        ...
        for pr in prs:
            comments = format_issue_to_csv(pr)
            closingIssues1 = [issue for issue in issues if issue.number in pr.closingIssuesReferences_numbers] 
            closingIssues2 = [issue for issue in issues if issue.number in pr.sourceIssuesReferences_numbers] 
            closingIssues3 = [issue for issue in issues if pr.number in issue.sourceIssuesReferences_numbers]
            closingIssues4 = [issue for issue in issues if pr.number in issue.associatedPullRequests_numbers]
            closingIssues = closingIssues1 + closingIssues2 + closingIssues3 + closingIssues4
            closingIssuesNumbers = list(set([issue.number for issue in closingIssues]))
            closingIssues = [issue for issue in issues if issue.number in closingIssuesNumbers]
            logger.debug(f"PR {pr.number}: closing issues from different sources")

            all_text = ""
             # Process the comments for this issue
            for closingIssue in closingIssues:
                all_text += "".join(format_issue_to_csv(closingIssue))

            latestReviews = ""
            latestReviews += "".join(pr.latestReviews)

            closingIssuesReferences = ", ".join([f"#{number}" for number in closingIssuesNumbers])
            pr_commits_str = ", ".join(pr.pr_commits_ids) if pr.pr_commits_ids else ""
            merged_commits_str = ", ".join(pr.merged_commits_ids) if pr.merged_commits_ids else ""
            parents = ", ".join(pr.parents_ids) if pr.parents_ids else ""

            writer.writerow({
                'number': pr.number,
                'type': 'pr',
                'state': pr.state,
                'pr_commits': pr_commits_str,
                'merged_commits': merged_commits_str,
                'parents': parents,
                'issues': closingIssuesReferences,
                'comments': comments + all_text + latestReviews
            })
...
def main():
    ...
    if not os.path.exists(local_dir):
        logger.info("Downloading github repository locally")
